<template>
  <div class="edit_form">
    ...
    <edu-form :title="textMap[dialogStatus]" @ok="dialogStatus === 'create' ? createData() : updateData()" @cancel="handelCancel">
      <a-form-model ref="veStuHealthReportDataForm" :rules="veStuHealthReportRules" :model="veStuHealthReportVo" labelAlign="right">
        ...
        <a-form-model-item label="开始时间" prop="startTime" required>
          <a-date-picker v-model="veStuHealthReportVo.startTime" valueFormat="YYYY-MM-DD" />
        </a-form-model-item>
        <a-form-model-item label="结束时间" prop="endTime" required>
          <a-date-picker v-model="veStuHealthReportVo.endTime" valueFormat="YYYY-MM-DD" />
        </a-form-model-item>
        <a-form-model-item label="健康体质得分" prop="score" required>
          <a-input-number v-model="veStuHealthReportVo.score" :min="0" :max="1000" placeholder="请输入健康体质得分" style="width:300px" />
        </a-form-model-item>
      </a-form-model>
    </edu-form>
  </div>
...
export default {
  ...
  data() {
    ...
    const data = {
      ...
      xm: '',
      dialogStatus: 'create',
      veStuHealthReportRules: { userIds: [{ validator: validateUserZy, trigger: 'change', required: true }] }
    };
    return data;
  },
  mounted() {},
