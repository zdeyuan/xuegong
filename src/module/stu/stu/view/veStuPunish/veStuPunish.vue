<template>
  <edu-layout type="table" :extHeight="viewType == 'count' ? 50 : 0" topName="惩罚查询">
    ...
    <div slot="table" class="right_table">
      <div>
        <edu-query ext="boxZybmbj" :nianji="false" ref="cwQuery" code="veStuPunish" :do-filter="handleFilter">
          <template slot="search">
            <edu-query-item title="惩罚类型" pname="d.type_id" textKey="title" pop="eq" stype="select" :datas="typeNodes" dtype="datas">
            </edu-query-item>
            <edu-query-item title="姓名" pname="name" pop="like" stype="input"> </edu-query-item>
            <edu-query-item title="学号" pname="d.code" pop="like" stype="input"> </edu-query-item>
          </template>
        </edu-query>
        <edu-table
          ...
          :isAutoBtn="viewType == 'count'"
          :to-update="handleUpdate"
          :do-add="viewType == 'count' ? null : handleCreate"
        >
...
import veStuPunishTypeApi from '@/module/stu/stu/api/veStuPunishTypeApi';
export default {
  props: {
    viewType: String
  },
