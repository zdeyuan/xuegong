 ...
 * @LastEditTime: 2021-05-01 19:00:20
-->
<template>
  <div class="edit_form">
    <!-- 编辑页 -->
    <edu-form title="保险理赔咨询详情" @ok="createDate" @cancel="handelCancel">
      <div>
        <div style="padding:15px">
          <h2 style="text-align:center"> {{ veStuInsuranceAskVo.title }}</h2>
        </div>
        <div style="padding:15px">
          <h3> {{ veStuInsuranceAskVo.content }}</h3>
        </div>
...
import veStuInsuranceReplyApi from '@/module/stu/bxxl/api/veStuInsuranceReplyApi';
export default {
  components: {},
  data() {
    const data = {
  ...
  methods: {
    init() {
      const sub = {
        conditions: [{ operator: 'eq', column: 'ask_id', value: this.veStuInsuranceAskVo.id }]
      };
      veStuInsuranceReplyApi.all(sub).then(res => {
     ...
     */
    doShow(row) {
      this.veStuInsuranceAskVo = Object.assign({}, row);
      this.init();
    ...
    getSubData(isUpdate) {
      const tempData = {
        ...
        askId: this.veStuInsuranceAskVo.id // 咨询内容Id
      };
      return tempData;
    },
    /**
    ...
    createDate() {
      ...
      veStuInsuranceReplyApi.add(subData).then(res => {
